...
}).with_columns(pl.col('decimal').cast(pl.String).str.to_decimal(scale=1))
...
def test_polars_transpose_description():
    _pl.transpose_description(df.describe(percentiles=(0.25, 0.75)))
...
def test_polars_summary(group, transpose, tmp_path):
    summ = _pl.PolarsSummary(
        df, group=group, percentiles=(0.25, 0.75), transpose=transpose
    )
    summ.describe()
